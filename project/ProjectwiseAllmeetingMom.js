...
import ProjectMomView from './ProjectMomView';
...
const ProjectwiseAllmeetingMom = ({ project, projectId }) => {
  ...
  const { moms, momsLoading, momsError, selectedMom, selectedMomLoading, selectedMomError } = useSelector(
  ...
  return (
    <div className="mx-auto space-y-6">
      ...
      <Dialog open={!!viewingMom} onOpenChange={() => setViewingMom(null)}>
        <DialogContent className="max-w-full sm:max-w-3xl max-h-[90vh] overflow-y-auto bg-white rounded-md p-4 sm:p-6 shadow-lg">
          <DialogHeader>
            <DialogTitle className="text-lg sm:text-xl font-semibold text-gray-800 flex items-center gap-2">
              <FileText className="h-5 w-5 text-indigo-500" /> Meeting Minute Details
            </DialogTitle>
          </DialogHeader>
          <ProjectMomView momData={viewingMom} momId={viewingMom?.momId} onClose={() => setViewingMom(null)} />
        </DialogContent>
      </Dialog>
    </div>
  );
