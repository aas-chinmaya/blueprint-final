...
// import { Plus, Search, Calendar as CalendarIcon, Video, AlertTriangle, Loader2, Eye, Edit, Trash2, FileText, Users, Download, ExternalLink, X, List, Clock, User } from 'lucide-react';
...
//   const currentUser = userData?.fullName; // Replace with actual user data from auth context or state

//   const { moms, momsLoading, momsError, selectedMomLoading, selectedMomError, deleteSuccess, meetingMomView, meetingMomViewLoading, meetingMomViewError } = useSelector((state) => state.projectMeetingMom);
...
//   }, [dispatch, projectId, searchQuery, filters]);
...
//         ? {
//             ...editingMom,
//             projectName: projectName || editingMom.projectName,
//             projectId: projectId || editingMom.projectId,
//             meetingDate: editingMom.meetingDate && isValid(parseISO(editingMom.meetingDate)) ? parseISO(editingMom.meetingDate) : null,
//             startTime: editingMom.meetingDate && isValid(parseISO(editingMom.meetingDate)) ? format(parseISO(editingMom.meetingDate), 'HH:mm') : '',
//             endTime: editingMom.endTime && isValid(parseISO(editingMom.endTime)) ? format(parseISO(editingMom.endTime), 'HH:mm') : '',
//             duration: editingMom.duration || '',
//             createdBy: currentUser,
//             status: editingMom.status || 'draft',
//           }
//         : {
//             title: '',
//             projectName: projectName || '',
...
//     );
//     setFormErrors({});
//   }, [editingMom, showCreateForm, projectName, projectId, currentUser]);
...
//     try {
//       editingMom
//         ? await dispatch(updateProjectMeetingMom({ momId: editingMom.id, updatedData: data })).unwrap()
...
//       link.download = `meeting-minute-${meetingMomView.momId}.pdf`;
...
//   const totalPages = Math.ceil((moms?.data || []).length / itemsPerPage);
//   const paginatedMoms = (moms?.data || []).slice(
...
//         </Badge>
//         <Badge
//           variant={mom.status === 'final' ? 'default' : 'secondary'}
//           className="text-xs"
//         >
//           {mom.status || 'Draft'}
//         </Badge>
...
//                   <FileText className="h-4 w-4 text-indigo-500" /> Meeting Title *
//                 </Label>
//                 <Input
//                   id="title"
//                   name="title"
...
//                     <Video className="h-4 w-4 text-indigo-500" /> Meeting ID *
//                   </Label>
//                   <Input
//                     id="meetingId"
//                     name="meetingId"
